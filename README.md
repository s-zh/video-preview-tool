...
| 在源视频同目录下生成相应预览图 | 关闭 | 勾选后预览图保存在源视频目录（默认仅在临时目录，通过 ZIP 下载） |

### 5. 生成预览
点击"生成预览图"创建任务，在任务详情中可实时查看处理进度和每个文件的状态。

### 6. 结果管理
- **预览** — 任务完成后点击缩略图可放大查看预览图
- **下载** — 点击"下载预览图 (ZIP)"打包所有预览图
- **重试** — 失败的文件可在任务详情中一键重试
- **删除** — 清理任务列表。预览图保存在临时目录的会一同删除，保存在源视频目录的不受影响
...
| `POST` | `/api/tasks/cleanup` | 清理所有已完成/已取消的任务 |
| `GET` | `/api/download/{task_id}` | 下载任务的预览图 ZIP 包 |
| `GET` | `/api/preview/{task_id}/{index}` | 获取任务第 N 张预览图（用于前端展示） |
